...
if !isdefined(@__MODULE__, :CHEME5800_L10B_ROOT)
    const CHEME5800_L10B_ROOT = @__DIR__
end
...
include(normpath(joinpath(CHEME5800_L10B_ROOT, "..", "..", "..", "Include.jl")))

# `Week10Core` holds this meeting's own source. It is wrapped in a module so that
...
if !isdefined(@__MODULE__, :Week10Core)
    include(normpath(joinpath(CHEME5800_L10B_ROOT, "..", "src", "Week10Core.jl")))
end
...
using Distributions  # probability distributions and sampling
using Plots          # figures
...
using .Week10Core    # from the include in section 2
