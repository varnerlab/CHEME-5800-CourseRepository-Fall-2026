# Student implementation for L5d. Complete the three TODOs in
# flow_formulation(...), save, restart the notebook kernel, and run from the top.
...
function flow_formulation(edges::AbstractVector{FlowEdge}, source::Integer, sink::Integer, required_flow::Real)
    ...
    # Initialize the incidence matrix and required balances -
    A = zeros(Float64, length(vertices), length(edges)) # rows: nodes; columns: edges
    b = zeros(Float64, length(vertices)) # net inflow required at each node

    # TODO 1: For each edge j, put -1 in its source row and +1 in its target row.
    # Use row[edge.source] and row[edge.target] to locate the two rows.

    # TODO 2: Set the source balance to -required_flow and the sink balance to
    # +required_flow. Leave intermediate-node balances at zero.

    # TODO 3: Return A, b, c, lower, upper, and vertices in a named tuple.
    # Read c, lower, and upper from the edge records in their input order.

    throw(ErrorException("Oooops! The `flow_formulation(...)` function is not implemented yet - we'd better fix that."))
end
