...
Returns a `DataFrame` with one row per faculty → course edge whose flow exceeds `atol` (in assignments), in course order. The columns are `course`, `title`, `name`, `score`, `flow`, and `fixed` (whether the department fixed that assignment). Flows are reported as returned, without rounding; checking that they are whole assignments is a separate step.
