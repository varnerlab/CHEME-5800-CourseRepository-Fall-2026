...
\begin{frame}{Objectives for Today}
  ...
  \begin{itemize}\setlength{\itemsep}{0.7em}
    ...
    \item \hilite{Verify maximum flow}: check feasibility and use a cut
      capacity to prove that more flow is impossible.
  \end{itemize}
...
\begin{frame}{Maximize Net Flow Leaving the Source}
  ...
  \]
  Subtract returning flow. Conservation makes $|f|$ equal to the net inflow
  at the sink $t$.

  \vspace{0.5em}
  The maximum-flow problem seeks a feasible flow $f^*$ with largest value:
  \[
    |f^*|=\max_{f\;\text{feasible}}|f|.
  \]

  \slidenote{Worker–task model:}{no edges enter the source, so its total outflow counts completed assignments.}
\end{frame}

% Notebook: Ford-Fulkerson Method; What is a Residual Graph?
\begin{frame}{Ford–Fulkerson Method}
  Start with zero flow. Repeatedly find a source-to-sink route that allows
  the total flow to increase.

  \vspace{0.8em}
  A \hilite{residual graph} records changes we can make to the current flow:
  ...
  \vspace{0.7em}
  Ford–Fulkerson is a \hilite{method}: it leaves the path-selection rule open.
\end{frame}
...
\begin{frame}{Defining Residual Capacities}
  ...
  \slidenote{Scope:}{the worker–task network satisfies these assumptions; $f$ remains nonnegative on original edges.}
\end{frame}
...
% Notebook: How do we find augmenting paths? Edmonds–Karp, then DFS.
...
\begin{frame}{Depth-First Search and Termination}
  \hilite{Depth-first search} finds a residual path; it need not have the
  fewest edges.

  \vspace{0.8em}
  For nonnegative integer capacities and zero initial flow:
  \begin{itemize}\setlength{\itemsep}{0.7em}
    \item Each augmentation adds at least one unit of flow.
    \item Finite source capacity bounds the total flow.
    \item The method terminates; the iteration count can depend on capacities.
  \end{itemize}

  \vspace{0.7em}
  Irrational capacities can allow a sequence of path choices that never terminates.
\end{frame}

\begin{frame}{Compare Both Searches on the Same Network}
  The \href{\examplelink}{worker–task example} applies two path-selection
  rules to the same capacities.

  \vspace{0.9em}
  \renewcommand{\arraystretch}{1.4}
  \begin{tabularx}{\textwidth}{YY}
    \toprule
    \textbf{Course solver} & \textbf{Residual-path selection}\\
    \midrule
    Ford–Fulkerson implementation & Depth-first search\\
    Edmonds–Karp & Breadth-first search; fewest edges\\
    \bottomrule
  \end{tabularx}

  \vspace{0.9em}
  Both methods should give the same maximum-flow value. Their individual
  worker–task assignments may differ.

  \vspace{0.7em}
  We next check feasibility and establish that the value is maximum.
\end{frame}

% Notebook: Verifying feasibility and maximum flow.
\begin{frame}{Verifying Feasibility and Maximum Flow}
  Independently check the reported edge flows:
  ...
  \vspace{0.8em}
  These checks establish feasibility. To prove the flow is maximum, find
  an upper bound that the feasible flow reaches.
\end{frame}
...
\begin{frame}{A Source–Sink Cut Gives an Upper Bound}
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      Cut capacity sums original capacities from $S$ to $T$:
      \[
        c(S,T)=\sum_{\substack{(u,v)\in\mathcal E\\u\in S,\;v\in T}}c(u,v).
      \]
      Here the two \hilite{outgoing cut edges} give $c(S,T)=1+1=2$.

      \vspace{0.6em}
      The \textcolor{vnslidelink}{return edge} adds no cut capacity.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Cut Balance Bounds Every Feasible Flow}
  ...
  In the illustrated cut, $|f|=2-1=1\leq c(S,T)=2$.
  The return flow reduces net delivery.
\end{frame}
...
\begin{frame}{No Augmenting Path Identifies a Cut}
  ...
  \vspace{0.4em}
  Residual augmentation $s\to b\to t$ cancels $b\to s$ and gives $|f|=2$.
\end{frame}
...
\begin{frame}{Three Assignments Reach the Source-Cut Bound}
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.49\textwidth}
      Put source 1 in $S$ and every other vertex in $T$. The cut capacity is:
      \[
      ...
      \[
        |f|=c(S,T)=3.
      \]
      Therefore, \hilite{three assignments} is the maximum.
    \end{column}
  \end{columns}
...
\begin{frame}{Summary}
  ...
  \begin{itemize}\setlength{\itemsep}{0.6em}
    ...
    \item \textbf{\ink{Residual augmentation.}} We used bottlenecks to add
      or cancel flow while preserving feasibility. Edmonds–Karp chose paths
