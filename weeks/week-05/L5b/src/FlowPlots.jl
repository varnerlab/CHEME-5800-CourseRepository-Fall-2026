# =============================================================================
# CHEME 5800 | L5b plotting helper
# =============================================================================
# Draws the worker–task flow network for the L5b lab. Adapted from the L5a
# worked example's helper: edges whose capacity differs from the common value are
# labeled, so a single expanded or blocked edge stands out. The meeting's
...
Draw the 13-node worker–task network used in the L5b lab.
...
# Returns
A `Plots.Plot` object. The most common capacity is stated once in the caption;
an edge whose capacity differs from it is labeled. When a flow is supplied, draw
positive-flow edges in red and label them with `flow / capacity`; gray edges
carry zero flow. The graph, coordinates, and flow dictionary are not modified.
...
function plot_flow_network(graph, coordinates; flow = nothing, title = "Flow network", theme = :auto)
    ...
    sort(collect(keys(graph.nodes))) == collect(1:13) ||
        throw(ArgumentError("this drawing uses the L5b node identifiers 1:13"))
    ...
    # The most common upper capacity is stated once; other edges are labeled -
    counts = Dict{Any, Int}()
    for upper in capacities
        counts[upper] = get(counts, upper, 0) + 1
    end
    reference_capacity = argmax(counts)
    common_capacity = all(==(reference_capacity), capacities)
    ...
    for edge in edges
        ...
        if active || upper != reference_capacity
            ...
            # Place the label just above the edge: a third of the way along a
            # flow-carrying edge, and farther along a zero-flow edge so the two
            # kinds of label do not collide where edges cross -
            along = active ? 0.35 : 0.7
            annotate!(figure, x + along * dx, y + along * dy + 0.14, text(label, 10, color))
        end
    end
    ...
    capacity_label = isinteger(reference_capacity) ? string(Int(reference_capacity)) :
        string(round(reference_capacity; digits = 2))
    unit_label = reference_capacity == 1 ? "assignment" : "assignments"
    if common_capacity
        caption = "Every edge has a capacity of " * capacity_label * " " * unit_label * "."
    else
        caption = "Unlabeled edges have a capacity of " * capacity_label * " " * unit_label *
            "; other capacities are labeled."
    end
    if show_flow
