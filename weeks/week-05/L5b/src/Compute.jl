# Flow-validation and cut-capacity helpers for L5b. The lab walks through these
# checks in Task 1; the implementation is supplied so the notebook runs from the
# top. An optional exercise: delete the body of validate_sensitivity_flow(...) and
# rebuild it from its docstring, then compare with Compute-solution.jl.
