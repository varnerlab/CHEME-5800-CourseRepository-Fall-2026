...
# `Week00Toolchain` holds this meeting's own source. It is wrapped in a module so that
...
if !isdefined(@__MODULE__, :Week00Toolchain)
    include(joinpath(@__DIR__, "src", "HelloWorld.jl"))
end
...
#
# Packages:
using UnicodePlots      # plots drawn in the terminal, no display backend needed
#
...
using .Week00Toolchain  # from the include in section 2
