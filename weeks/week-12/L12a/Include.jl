...
if !isdefined(@__MODULE__, :CHEME5800_L12A_ROOT)
    const CHEME5800_L12A_ROOT = @__DIR__
end
...
include(normpath(joinpath(CHEME5800_L12A_ROOT, "..", "..", "..", "Include.jl")))

# `Week12Core` holds this meeting's own source. It is wrapped in a module so that
...
if !isdefined(@__MODULE__, :Week12Core)
    include(normpath(joinpath(CHEME5800_L12A_ROOT, "..", "src", "Week12Core.jl")))
end
...
using .Week12Core    # from the include in section 2
