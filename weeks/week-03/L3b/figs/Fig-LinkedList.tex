...
% The notebook supplies the SVG's accessible description through its <img> alt text.
\documentclass[tikz,border=10pt]{standalone}
\usetikzlibrary{arrows.meta, calc}
\definecolor{chemered}{HTML}{EF4035}
\definecolor{chemewheat}{HTML}{FAEEE2}
\definecolor{linkgray}{HTML}{6B7280}
\begin{document}
\begin{tikzpicture}[
    ...
    % ================= row 2: the splice =================
    \node[note, anchor=east] (head2) at (-1.1, -1.55) {head};

    \node[vbox] (a2)  at (0, -1.55) {$2$};
    \node[pbox, anchor=west] (a2p) at (a2.east) {};
    \node[vbox] (b2)  at (2.6, -1.55) {$4$};
    \node[pbox, anchor=west] (b2p) at (b2.east) {};
    \node[vbox] (c2)  at (5.2, -1.55) {$8$};
    ...
    \draw[ref, linkgray, dash pattern=on 3pt off 2.2pt] (b2p.center) -- (c2.west);
    \coordinate (oldmid) at ($(b2p.center)!0.52!(c2.west)$);
    \draw[thick, linkgray] ($(oldmid)+(-0.11,-0.11)$) -- ($(oldmid)+(0.11,0.11)$);
    \draw[thick, linkgray] ($(oldmid)+(-0.11,0.11)$) -- ($(oldmid)+(0.11,-0.11)$);
    \node[note, text=linkgray, anchor=south, font=\footnotesize]
        at ($(oldmid)+(0.08,0.20)$) {old link};

    % the new node, spliced in with two relinks and no shifting -
    \node[vbox, fill=chemewheat] (m)  at (3.4, -2.95) {$16$};
    ...
    \node[note, align=center, anchor=north] at (2.95, -3.75)
