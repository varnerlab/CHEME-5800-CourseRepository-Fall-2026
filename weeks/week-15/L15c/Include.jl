...
if !isdefined(@__MODULE__, :CHEME5800_L15C_ROOT)
    const CHEME5800_L15C_ROOT = @__DIR__
end
...
include(normpath(joinpath(CHEME5800_L15C_ROOT, "..", "..", "..", "Include.jl")))

# `Week15Core` holds this meeting's own source. It is wrapped in a module so that
...
if !isdefined(@__MODULE__, :Week15Core)
    include(normpath(joinpath(CHEME5800_L15C_ROOT, "..", "src", "Week15Core.jl")))
end
...
using .Week15Core     # from the include in section 2
