...
\begin{frame}{Connectivity Depends on Direction}
  ...
  \vspace{0.5em}
  A \hilite{directed acyclic graph} has no directed cycles.
\end{frame}

\begin{frame}{Direction Changes Which Routes Are Feasible}
  \begingroup\centering
    \makebox[0.5\textwidth]{\textbf{\ink{Undirected graph}}}%
    \makebox[0.5\textwidth]{\textbf{\ink{Directed acyclic graph}}}\par
    \vspace{0.1em}
    % Crop the asset's header and outer whitespace; set the panel titles above.
    \includegraphics[width=\textwidth,trim=45bp 84bp 30bp 120bp,clip]{assets/Fig-General-Graph-Schematic.pdf}\par
  \endgroup

  Undirected edges allow travel either way; directed edges follow the arrows.

  \vspace{0.1em}
  The directed graph is acyclic and weakly connected, but not strongly connected.
\end{frame}
...
\begin{frame}{Beyond Vertex and Edge Counts}
  A path graph forms a single chain. A star graph connects every outer vertex
  to one central vertex. For $n\geq4$, both have $n$ vertices and $n-1$ edges.

  \vspace{0.8em}
  Their average degree and density agree, but distances between vertices differ.
  For a connected undirected graph, the \hilite{diameter}
  $\operatorname{diam}(\mathcal G)$ is the largest shortest-path distance,
  measured in edges.

  \vspace{0.8em}
  We compare a path and a star on six vertices to see what edge counts alone
  cannot tell us.
\end{frame}
...
\begin{frame}{Storage and Access Costs}
  ...
  \slidenote{Scan assumption:}{the adjacency-list edge search scans the source vertex's neighbor vector.}
\end{frame}
...
\begin{frame}{Worked Example: A Six-Vertex Directed Graph}
  ...
  The connections match the earlier directed graph; labels $0$--$5$ become $1$--$6$.
...
\begin{frame}{Complete-Graph Measures Follow Directly}
  For $n\geq2$, every vertex meets $n-1$ others. Counting both endpoints of
  each edge gives:
  \[
    |\mathcal E|=\binom n2=\frac{n(n-1)}{2},\qquad
    \deg(v_i)=n-1.
  \]
  Every possible edge is present, and each pair of distinct vertices is one edge apart:
  \[
    \rho(K_n)=1,\qquad \operatorname{diam}(K_n)=1.
  \]

  These counts describe connectivity and distance. Next, we consider groups
  of vertices and the colors needed to distinguish adjacent vertices.
\end{frame}

\begin{frame}{Clique, Chromatic, and Independence Numbers}
  ...
  Every pair in $K_n$ is adjacent, so for $n\geq2$:
  \[
...
\begin{frame}{Bipartite and Complete Bipartite Graphs}
  A bipartite graph has edges only between two disjoint vertex groups.
  The complete bipartite graph $K_{m,n}$ includes every edge between them.
...
\begin{frame}{Three Equivalent Tests Identify Bipartite Graphs}
  ...
  the test takes worst-case $\Theta(|\mathcal V|+|\mathcal E|)$ time.
\end{frame}
...
\begin{frame}{Bipartite Matrices Have Block Structure}
  ...
  The $m\times n$ \hilite{biadjacency matrix} $\mathbf B$ records connections
  from $\mathcal V_1$ to $\mathcal V_2$. For an undirected graph:
  \[
  ...
  In a directed graph, an $n\times m$ matrix $\mathbf C$ records the reverse
  connections, which can differ:
  \[
    \mathbf A=\begin{bmatrix}
      \mathbf 0 & \mathbf B\\
      \mathbf C & \mathbf 0
    \end{bmatrix}.
  \]
  The diagonal blocks are zero because no edge stays within a group.
  If every edge points from $\mathcal V_1$ to $\mathcal V_2$, then $\mathbf C=\mathbf 0$.

  \slidenote{Undirected storage:}{for $K_{3,4}$, $\mathbf B$ is a $3\times4$ matrix of ones; $\mathbf B$ and the vertex order determine $\mathbf A$.}
\end{frame}

\begin{frame}{A Matching Covers the Smaller Group}
  A \hilite{matching} is a set of edges that share no vertices. Here, the
  highlighted edges cover $\mathcal V_1$, leaving $v_4$ unmatched.

  \vspace{0.15em}
  \begin{center}
    \includegraphics[width=0.94\textwidth,height=0.65\textheight,keepaspectratio]{../figs/Fig-Bipartite-Graph-Schematic/Fig-Bipartite-Graph-Schematic-panel.pdf}
  \end{center}
\end{frame}

\begin{frame}{Matchings and Hall's Theorem}
  A matching that covers $\mathcal V_1$ assigns a different right-side option
  to every left-side vertex. Every group on the left must have enough neighbors
  to receive distinct assignments.
...
\begin{frame}{Tree Edge Counts, Density, and Storage}
  Start with $n\geq1$ isolated vertices. Each tree edge joins two components;
  joining vertices already connected would create a cycle.
  Reaching one component therefore requires $m=n-1$ edges.

  \vspace{0.6em}
  For $n\geq2$, the tree's density is given by:
  \[
...
\begin{frame}{Reference: Equivalent Tree Characterizations}
  For a simple undirected graph with $n\geq1$, these statements are equivalent:
  ...
  \begin{enumerate}\setlength{\itemsep}{0.5em}
    ...
    \item It has no cycle, and adding any missing edge creates exactly one cycle.
    \item Every pair of vertices is joined by exactly one path.
  \end{enumerate}

  \vspace{0.5em}
  Once connectivity is known, an edge count of $n-1$ is enough to establish
