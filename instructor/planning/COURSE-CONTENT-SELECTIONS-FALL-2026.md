...
Reuse the instructional sequence from CHEME-142 Module 4: host/client/server roles, schema-driven discovery, valid and invalid calls, the distinction between computation and protocol tests, and explicit trust boundaries. Its handwritten Julia MCP server remains a useful deeper dive, but it targets protocol revision `2025-06-18` and is not the Fall 2026 common implementation. The common implementation uses the official Python SDK v2 and adds the read-only resource that CHEME-142 discussed but did not implement.
...
The opening lecture uses a small explicit-Euler implementation to make time stepping, local error, step-size sensitivity, and stability concrete, then contrasts it with a maintained solver package. Week 15 is a complete four-meeting instructional block: a three-gene implementation lab, a state-update/CHEME 5820 bridge lecture, and a maintained-solver fermentation lab follow the opening lecture. It remains a compact numerical-dynamics unit rather than a multi-week differential-equations course.
...
| Dynamic simulation | 2024 three-gene and fermentation labs | Two Week 15 engineering labs |
...
| Week 15 dynamics block | Adapt the 2024 three-gene and fermentation activities into two lectures and two labs |
| Deeper dives | Curate verified sources only; no blanket authoring program |

The Week 13 communications unit served as the first technical risk prototype because it exercises Python packaging, fixtures, tests, documentation, and cross-language course data. The chronological Weeks 1–16 build is now complete; the remaining work is cross-course cleanup and release preparation.
